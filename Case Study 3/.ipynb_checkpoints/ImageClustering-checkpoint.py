...
class ImageClustering:
    ...
    def cluster_image(self, n_clusters=None, dbscan_params={}, preprocess_kwargs={}):
        ...
        # Handle the case where some labels are -1 (considered as noise in DBSCAN)
        n_clusters = len(set(labels)) - (1 if -1 in labels else 0)
        cluster_colors = np.zeros((n_clusters, 3), dtype=np.uint8)
        
        for label in np.unique(labels):
            if label != -1:  # Ignore noise
                cluster_colors[label] = np.mean(original_pixels[labels == label], axis=0)
            else:
                cluster_colors = np.insert(cluster_colors, 0, [0, 0, 0], axis=0)  # Insert black for noise
